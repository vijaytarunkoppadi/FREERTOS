...
void Sender(void *params)
{
  while (true)
  {
    xEventGroupWaitBits(event_group, got_temp | got_hum, 
                        true,                               // if true clear the bit before return function
                                                            // if false set, event group not changed
                        true,                               // if true , wait for max_wait_time of all bits 
                                                            // if false , any bit set or Max_block_time occurs
                        portMAX_DELAY);    
